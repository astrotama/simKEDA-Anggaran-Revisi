...
drupal_add_css('files/css/kegiatancam.css');
	...
	if (isset($exportpdf) && ($exportpdf=='pdf'))  {
		...
		$htmlContent = GenReportFormContent($kodeuk, $jenis, $detil);
		
		apbd_ExportPDF3P($topmargin,$topmargin, $htmlHeader, $htmlContent, '', $pdfFile);
		
	} else {
		$url = 'apbd/laporanpenetapan/rka/rekapplafonagg/' . $topmargin . "/pdf";
		...
		$output .= GenReportForm($kodeuk, $jenis, $detil);
		return $output;
	}

}

function GenReportForm($kodeuk, $jenis, $detil) {
...
function GenReportFormContent($kodeuk, $jenis, $detil) {
...
function rekapplafonagg_form_submit($form, &$form_state) {
	...
	if($form_state['clicked_button']['#value'] == $form_state['values']['tampilkan'])
		$uri = 'apbd/laporanpenetapan/rka/rekapplafonagg/'. $kodeuk . '/'. $jenis . '/' . $detil . '/'.  $topmargin . '/' ;
	else
		$uri = 'apbd/laporanpenetapan/rka/rekapplafonagg/'. $kodeuk . '/'. $jenis . '/' . $detil . '/'.  $topmargin . '/pdf' ;
	drupal_goto($uri);
