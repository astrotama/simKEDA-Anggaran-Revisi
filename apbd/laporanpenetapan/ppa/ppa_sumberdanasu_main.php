<?php
function ppa_sumberdanasu_main() {
...
drupal_add_css('files/css/kegiatancam.css');
	...
	if (!isset($tahun)) 
		return drupal_get_form('ppa_sumberdanasu_form');
	...
	if (isset($exportpdf) && ($exportpdf=='pdf'))  {
		...
		ppa_sumberdanasu_exportexcel($kodeuk, $tahun);
		
	} else {
		//PDF
		$urlpdf = 'apbd/laporanpenetapan/ppa_sumberdanasu/' . $kodeuk . '/' . $tahun . '/pdf';
		$urlxls = 'apbd/laporanpenetapan/ppa_sumberdanasu/' . $kodeuk . '/' . $tahun . '/xls';
		$output .= drupal_get_form('ppa_sumberdanasu_form');
...
function GenReportForm($print=0) {
	...
	//SKPD
	$sql = 'select s.kodeuk, s.kodedinas, s.namasingkat, sum(k.apbdkab) jkab, sum(k.apbdprov) jprov, sum(k.apbdnas) jnas, sum(k.total) jtotal from kegiatanppa k inner join unitkerja s on k.kodeuk=s.kodeuk ' . $where . $wheresub . ' group by s.kodeuk, s.kodedinas, s.namasingkat order by s.kodedinas';
	...
	if ($result) {
		...
		while ($data = db_fetch_object($result)) {                
			...
			$sql = 'select r.kodeu, r.urusansingkat, sum(k.apbdkab) jkab, sum(k.apbdprov) jprov, sum(k.apbdnas) jnas, sum(k.total) jtotal from kegiatanppa k inner join program p on k.tahun=p.tahun and k.kodepro=p.kodepro inner join urusan r on p.kodeu=r.kodeu ' . $where . $wheresub . ' group by r.kodeu, r.urusansingkat order by r.kodeu';
...
function ppa_sumberdanasu_form () {
	$form['formdata'] = array (
		'#type' => 'fieldset',
...
function ppa_sumberdanasu_form_submit($form, &$form_state) {
	//$kodeuk = $form_state['values']['kodeuk'];
	$tahun = $form_state['values']['tahun'];
	$kodeuk = $form_state['values']['kodeuk'];
	$uri = 'apbd/laporanpenetapan/ppa_sumberdanasu/' . $kodeuk . '/' . $tahun;
	drupal_goto($uri);
	

}

function ppa_sumberdanasu_exportexcel($kodeuk, $tahun) {
...
if ($kodeuk !='00')	$qlike .= sprintf(' and k.kodeuk=\'%s\' ', $kodeuk);
...
$fname = 'sumberdana_ppas_skpd_urusan_' . $tahun . '.xlsx';
