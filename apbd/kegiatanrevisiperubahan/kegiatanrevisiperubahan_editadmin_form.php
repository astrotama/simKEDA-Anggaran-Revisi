<?php 
function kegiatanrevisiperubahan_editadmin_form() {

    $referer = $_SERVER['HTTP_REFERER'];
	if (strpos($referer, 'kegiatanrevisiperubahan/editadmin/')>0)
	...
	*/
	...
	//PREV
	$sql = 'select kodekeg from {kegiatanrevisi} where tahun=\'%s\' and kodeuk=\'%s\' and kodekeg<\'%s\' order by kodekeg desc limit 1';
	...
	//NEXT
	$sql = 'select kodekeg from {kegiatanrevisi} where tahun=\'%s\' and kodeuk=\'%s\' and kodekeg>\'%s\' order by kodekeg limit 1';
	$res = db_query(db_rewrite_sql($sql), array ($tahun, $kodeuk, $kodekeg));
	if ($res) {
		$data = db_fetch_object($res);
		if ($data) {
...
function kegiatanrevisiperubahan_editadmin_form_validate($form, &$form_state) {

$kodepro = $form_state['values']['kodepro'];
if ($kodepro == '') {
	form_set_error('Program', 'Program belum diisi' );
}  
	
}

function kegiatanrevisiperubahan_editadmin_form_submit($form, &$form_state) {
    if($form_state['clicked_button']['#value'] == $form_state['values']['submitnext']) {
		...
		{					
			...
			$nomorkeg =apbd_getcounterrevisi($kodekeg);
			$kodekeg .= $nomorkeg;
			...
			$res = db_query($sql);
			if ($res) {
				//drupal_set_message('new1');			  
				$sql =  sprintf("insert into {kegiatanrevisiperubahan} (jenisrevisi, subjenisrevisi, tahun, kodeuk, kodekeg) values('%s', '%s', '%s', '%s', '%s')", '4', '0', $tahun, $kodeuk, $kodekeg);
				//drupal_set_message($sql); 			
				$res = db_query($sql);
			}
			...
			$sql = sprintf("update {kegiatanrevisi} set kegiatan='%s', plafon='%s', sumberdana1='%s', sumberdana1rp='%s', kodeuk='%s', kodepro='%s', jenis='%s', inaktif='%s',  isppkd='%s',
		...
		if ($res) {
			...
			if ($kodeberubah) {
				...
				//Update anggperkegdetil
				$sql = sprintf("update {anggperkegdetilsubrevisi} set kodekeg='%s' where kodekeg='%s'",
								db_escape_string($kodekeg),
								$e_kodekeg);		
				$res = db_query($sql);
