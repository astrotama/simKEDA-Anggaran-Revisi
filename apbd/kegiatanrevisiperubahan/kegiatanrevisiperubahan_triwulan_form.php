<?php
function kegiatanrevisiperubahan_triwulan_form() {
    $form['formdata'] = array (
        '#type' => 'fieldset',
    ...
    $kodekeg = arg(3);
	$asal = arg(4);
	//if (!isset($asal)) $asal = '0';
	
	//drupal_set_message($asal . ' - ' . $kodekeg);
	...
	*/
	...
	if ($asal=='0')
		$lt = '/apbd/kegiatanrevisiperubahan';
	else 
		$lt = '/apbd/kegiatanrevisi/rekening/' . $kodekeg . '/0';
	
	if (!isVerifikator()) {
		$form['formdata']['submit'] = array (
			'#type' => 'submit',
			'#suffix' => "&nbsp;<a href='" . $lt . "' class='btn_blue' style='color: white'>Tutup</a>",
			'#value' => 'Simpan'
		);
	}	
...
function kegiatanrevisiperubahan_triwulan_form_validate($form, &$form_state) {
...
function kegiatanrevisiperubahan_triwulan_form_submit($form, &$form_state) {
    if($form_state['clicked_button']['#value'] == $form_state['values']['submitnext']) {
		...
		if ($asal=='0')
			...
			drupal_goto('apbd/kegiatanrevisi/rekening/' . $kodekeg . '/0');    
