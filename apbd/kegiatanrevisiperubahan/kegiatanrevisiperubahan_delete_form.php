...
function kegiatanrevisiperubahan_delete_form() {
    ...
    if (isset($kodekeg)) {
        ...
        $sql = 'select kodekeg, kegiatan, plafon from {kegiatanrevisi} where kodekeg=\'%s\'' . $customwhere;
...
function kegiatanrevisiperubahan_delete_form_validate($form, &$form_state) {
}
function kegiatanrevisiperubahan_delete_form_submit($form, &$form_state) {
    if ($form_state['values']['confirm']) {
        $kodekeg = $form_state['values']['kodekeg'];
        $sql = 'DELETE FROM {kegiatanrevisi} WHERE kodekeg=\'%s\' ';
        $res = db_query(db_rewrite_sql($sql), array($kodekeg));
        if ($res) {
