...
function kegiatanskpd_edit_form() {
	...
	*/
    ...
    $sql = 'select kodekeg,kodero,uraian,jumlahsebelum,jumlah,jumlahp,jumlahsesudah from {anggperkegperubahan}' . $where;
    ...
    if ($result) {
        while ($data = db_fetch_object($result)) {
			...
			if (user_access('kegiatanskpd penghapusan') and $allowedit)
                ...
                $rows[$no][0] = $no;
                $rows[$no][1] = $data->kodero;
				$rows[$no][2] = $uraian;
				$rows[$no][3] = apbd_fn($data->jumlah);
				$rows[$no][4] = apbd_fn($data->jumlahp);
				//$rows[$no][5] = $editlink;
	...
	$width=array(5,10,70,15,15);
	$posisi=array('center','right','left','right','right','center');
	for($n=0;$n<$no;$n++) 
	{
		 $form['tabledragrows'][$n] = array(
			'#title' => t('Rekening'), 
		...
		for($m=0;$m<5;$m++)
		{
			 $form['tabledragrows'][$n][$m]= array( 
				'#type'         => 'textfield', 
				//'#prefix' => '<div  style="text-align:center;">', 
				'#attributes'	=> array('style' => 'text-align: '.$posisi[$m]),
				'#value'=> $rows[$n][$m],
				'#size' => $width[$m],
				'#disabled'=> true,
				//'#weight' => 10,
    ...
    if($form_state['clicked_button']['#value'] == $form_state['values']['submitrek']) {
	   ...
	   $form_state['redirect'] = 'apbd/kegiatanskpdperubahan/print/' . $e_kodekeg  ;
		...
		if ($e_kodekeg=='')
		{
			...
			$sql = sprintf("update {kegiatanperubahan} set adminok=1, lokasi='%s', programsasaran='%s', programtarget='%s', masukansasaran='%s', 		
