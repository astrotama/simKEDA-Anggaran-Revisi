...
function pendapatan_main($arg=NULL, $nama=NULL) {
	...
	$allowedit = batastgl() || isUserview();//(($selisih>0) || (isUserview()));	
	...
	$sql = "select p.kodeuk,p.tahun,p.kodero,p.uraian,p.jumlah,p.jumlahp,p.jumlahsesudah,
			u.namasingkat,p.ketrekening,p.periode from {anggperuk} p left join {unitkerja} u on (p.kodeuk=u.kodeuk) " . $where;
	...
	if ($no==0) {
		...
		$rows[] = array (
			array('data' => 'Rekening belum diisikan', 'colspan'=>'8')
		);
	}
...
function pendapatan_main_form_submit($form, &$form_state) {
	...
	$uri = 'apbd/pendapatanr/filter/' . $kodeuk  . '/' . $kodek ;
	drupal_goto($uri);
