...
function rekeningrevisi_edit_form(){
	drupal_add_css('files/css/kegiatancam.css');
	//drupal_add_js('files/js/rekeningrevisibl.js'); 
    ...
    $form['formdata'] = array (
        '#type' => 'fieldset',
        '#title'=> 'Rekening Revisi Kegiatan',
        '#collapsible' => TRUE,
        '#collapsed' => FALSE,        
    );
	$kodekeg=arg(4);
	...
	//$title =l($title, 'apbd/kegiatanrevisi/rekening/' . $kodekeg, array('html'=>true));
    ...
    if (isset($kodero))
    {
        ...
        $sql = 'select kodekeg,kodero,uraian,jumlah,jumlahsebelum,jumlahsesudah from {anggperkegrevisi} where kodekeg=\'%s\' and kodero=\'%s\'';
		...
		if (!user_access('kegiatanskpd tambah'))
			drupal_access_denied();
		$form['formdata']['#title'] = 'Tambah rekeningrevisi Kegiatan';
		$kodero = '';
	}
	...
	$form['formdata']['kegiatan']= array(
		'#type'         => 'textfield', 
		'#title'        => 'rekeningrevisi', 
		//'#description'  => 'rekeningrevisi belanja', 
		'#maxlength'    => 255, 
		'#size'         => 70, 
		//'#required'     => !$disabled, 
		//'#disabled'     => $disabled, 
	...
	$form['formdata']['keterangan'] = array (
		'#type' => 'markup',
		'#value' => "<span><font size='1'>Isi rekeningrevisi dengan memilih menggunakan tombol Pilih</font></span>",
	);	
    ...
    $form['formdata']['submit'] = array (
        '#type' => 'submit',
		'#suffix' => "&nbsp;<a href='/apbd/kegiatanrevisi/rekening/" . $kodekeg . "' class='btn_blue' style='color: white'>Tutup</a>",
		//'#suffix' => "&nbsp;<a href='/apbd/kegiatanrevisi/rekening/' class='btn_blue' style='color: white'>Tutup</a>",
        '#value' => 'Simpan'
    );
    
    return $form;
}
function rekeningrevisi_edit_form_validate($form, &$form_state) {
	...
	if ($kodero=='') {		
		if ($uraian =='') {
			form_set_error('', 'rekeningrevisi belum diisi');
			
		} else {			//rekeningrevisi diisi dari mengetik
			...
			if ($res) {
				...
				if ($data) {
					...
					form_set_error('', 'rekeningrevisi tidak diisi/dipilih dengan benar');
				}
			} 
		}
	}
	
	if ($e_kodero != $kodero) {		//rekeningrevisi baru
		$sql = 'select kodero from {anggperkegrevisi} where kodekeg=\'%s\' and kodero=\'%s\'';
		$res = db_query(db_rewrite_sql($sql), array ($kodekeg, $kodero));
		if ($res) {
			$data = db_fetch_object($res);
			if ($data) {		
				form_set_error('', 'rekeningrevisi sudah digunakan dalam kegiatan ini');
			}
		}
	} 
	...
	if ($e_kodero=='') {
		...
		if ($uraian1 != '') {
			...
			$totalrekeningrevisibaru = $total1;
		}
		//CEK PER 1000
		//if (($total1 % $seribu)>0) form_set_error('', 'Isian detil rekeningrevisi #1 tidak bulat per seribu');
		...
		if ($uraian2 != '') {
			...
			$totalrekeningrevisibaru += $total2;
		}
		//CEK PER 1000
		//if (($total2 % $seribu)>0) form_set_error('', 'Isian detil rekeningrevisi #2 tidak bulat per seribu');
		...
		if ($uraian3 != '') {
			...
			$totalrekeningrevisibaru += $total3;
		}
		//CEK PER 1000
		//if (($total3 % $seribu)>0) form_set_error('', 'Isian detil rekeningrevisi #3 tidak bulat per seribu');
		...
		if ($uraian4 != '') {
			...
			$totalrekeningrevisibaru += $total4;
		}
		//CEK PER 1000
		//if (($total4 % $seribu)>0) form_set_error('', 'Isian detil rekeningrevisi #4 tidak bulat per seribu');
		...
		if ($uraian5 != '') {
			...
			$totalrekeningrevisibaru += $total5;
		}
		
		//CEK PER 1000
		if ($totalrekeningrevisibaru==0)
			form_set_error('', 'Isian rekeningrevisi belum dimasukkan dengan benar, anda harus mengisi detil rekeningrevisi sehingga jumlah anggaran rekeningrevisi ada nominal rupiahnya');
		if (($totalrekeningrevisibaru % $seribu)>0) form_set_error('', 'Jumlah isian detil rekeningrevisi tidak bulat per seribu');
		
		//CEK PLAFON
		/*
		$sql = sprintf("select total,plafon from {kegiatanrevisi} where kodekeg='%s'", $kodekeg);
		$result = db_query($sql);
		...
		if (($total+$totalrekeningrevisibaru)>$plafon) {		
			form_set_error('', 'Isian rekeningrevisi melebihi plafon, Plafon : ' . apbd_fn($plafon) . 
							   ', Sudah Masuk : ' . apbd_fn($total) . ', Isian Baru : ' . apbd_fn($totalrekeningrevisibaru) );
		}
		*/
...
function rekeningrevisi_edit_form_submit($form, &$form_state) {
	
	$kodekeg = $form_state['values']['kodekeg'];

    if($form_state['clicked_button']['#value'] == $form_state['values']['submitnext']) {
		$nextkode = $form_state['values']['nextkode'];
        $form_state['redirect'] = 'apbd/kegiatanrevisi/rekening/edit/' . $kodekeg . '/' . $nextkode ;
        ...
        $form_state['redirect'] = 'apbd/kegiatanrevisi/rekening/edit/' . $kodekeg . '/' . $prevkode ;
		//drupal_set_message('Next');

    } else if($form_state['clicked_button']['#value'] == $form_state['values']['submitrab']) {
		$kodero = $form_state['values']['nk'];
        $form_state['redirect'] = 'apbd/kegiatanrevisi/rekening/detil/' . $kodekeg . '/' . $kodero ;
		//drupal_set_message('Next');

	} else if($form_state['clicked_button']['#value'] == $form_state['values']['submitlist']) {
        $form_state['redirect'] = 'apbd/kegiatanrevisi/rekening/' . $kodekeg  ;
		//drupal_set_message('Next');
		...
		if (($kodero=='') and ($uraian !='')) {		//rekeningrevisi diisi dari mengetik
			$sql = 'select kodero from {rincianobyek} where uraian=\'%s\'';
			$res = db_query(db_rewrite_sql($sql), array ($uraian));
			if ($res) {
				$data = db_fetch_object($res);
		...
		if ($e_kodero=='')
		{
			...
			if ($uraian5 != '') {
				...
				$total5 = $unitjumlah5 * $volumjumlah5 * $harga5;
				$totalrekeningrevisi += $total5;
			}

			$sql = 'insert into {anggperkegrevisi} (kodekeg,kodero,uraian,jumlah,jumlahsebelum,jumlahsesudah) values(\'%s\', \'%s\', \'%s\', \'%s\', \'%s\', \'%s\')';        
			$res = db_query(db_rewrite_sql($sql), array($kodekeg, $kodero,$uraian, $totalrekeningrevisi, $jumlahsebelum, $jumlahsesudah));
			...
			$sql = 'update {anggperkegrevisi} set uraian=\'%s\', kodero=\'%s\', jumlahsebelum=\'%s\', jumlahsesudah=\'%s\' where kodekeg=\'%s\' and kodero=\'%s\'';
		...
		if($form_state['clicked_button']['#value'] == $form_state['values']['submitnew']) {
			$nextkode = $form_state['values']['nextkode'];
			//$form_state['redirect'] = 'apbd/kegiatanrevisi/rekening/edit/' . $kodekeg . '/' . $kodero . '/lanjut' ;
			$form_state['redirect'] = 'apbd/kegiatanrevisi/rekening/edit/' . $kodekeg . '/' . $nextkode  ;

			//$form['formdata']['submitnewdetil'] = array (
		
		} elseif ($form_state['clicked_button']['#value'] == $form_state['values']['submitnewdetil']) {
			$form_state['redirect'] = 'apbd/kegiatanrevisi/rekening/detil/edit/' . $kodekeg . '/' . $kodero  ;
