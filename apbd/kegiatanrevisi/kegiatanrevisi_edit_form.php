...
function kegiatanrevisi_edit_form() {
	...
	if (strpos($referer, 'kegiatanrevisi/edit/')>0)
    ...
    if (isset($kodekeg))
    {
        ...
        if ($res) {
			...
			if ($data) {    
				...
				$sql = 'select jenisrevisi, lokasi, sumberdana, kinerja, sasaran, detiluraian,rab, triwulan, lainnya
				...
				if ($res) {
					if ($data = db_fetch_object($res)) {
						$lokasi_r = $data->lokasi;
						$sumberdana_r = $data->sumberdana;
						$kinerja_r = $data->kinerja;
						$sasaran_r = $data->sasaran;
				...
				if ($res) {
					if ($data = db_fetch_object($res)) {
						$penetapan = $data->total;
						$plafon_lama = $data->plafon;
					}
				}		
	...
	$tipekeg = 'hidden';
	if (isSuperuser()) $tipekeg = 'textfield';
	...
	//drupal_set_message($kinerja);
	if (isSuperuser() or ($jenisrevisi_r==0) )		//SUPERUSER/BARU
		$tureq = false;
	else {
		if ($kinerja_r =='0') {
			$tuvisibe = 'hidden';
	...
	$kelompoksasarantype = 'textfield';
	if ($sasaran_r == '0') $kelompoksasarantype = 'hidden';
	$form['formdata']['kelompoksasaran']= array(
		'#type'         => $kelompoksasarantype, 
	...
	$lokasitype = 'item';
	if ($lokasi_r == 0) $lokasitype = 'hidden';
	$form['formdata']['lokasilabel']= array(
		'#type'         => $lokasitype, 
		'#title'        => 'Lokasi',
		'#description'  => 'Lokas kegiatan, anda bisa mengisi beberapa lokasi untuk satu kegiatan',
		'#value'		=> "<div id='lokasi' style='float:left'><span id='lokasilabel'></span><div id='btnTambah' style='float:left;'><a href='#bds' class='btn_green' style='color:#ffffff'>Tambah Lokasi</a></div></div><div style='clear:both'></div>"
		//'#cols'		=> '40',
		//'#rows'		=> '3',
		//'#disabled'     => true, 
		//'#description'  => 'lokasi', 
	...
	$form['formdata']['anggaran']['totalx']= array(
		'#type'         => 'textfield', 
		'#title'        => 'Jumlah', 
		'#description'  => 'Jumlah anggaran sebelumnya : ' . apbd_fn($penetapan),
		'#attributes'	=> array('style' => 'text-align: right'),
		'#size'         => 30, 
		//'#disabled'     => true, 
		'#default_value'=> $total, 
	...
	$form['formdata']['anggaran']['plafon']= array(
		'#type'         => 'textfield', 
		'#title'        => 'Plafon',
		'#description'  => 'Plafon anggaran sebelumnya : ' . apbd_fn($plafon_lama),
		'#attributes'	=> array('style' => 'text-align: right'),
		'#size'         => 30, 
		//'#disabled'     => true, 
	...
	*/
	...
	/*
	if ($adminok) {
		if ($jenisrevisi_r != 2 or $detiluraian_r==1) {
	...
	*/
	...
	//kegiatanskpdperubahan
	if ($allowedit) 
		$form['formdata']['submit'] = array (
			'#type' => 'submit',
			'#suffix' => "&nbsp;<a href='" . $referer . "' class='btn_green' style='color: white'>Tutup</a>",
			//'#suffix' => "&nbsp;<a href='/apbd/kegiatanskpdperubahan' class='btn_green' style='color: white'>Tutup</a>",
			'#value' => 'Simpan'
		);
...
function kegiatanrevisi_edit_form_validate($form, &$form_state) {
	...
	if ($e_kodekeg <> '') {
		...
		$sql = sprintf("select sum(total) as totalsub from {kegiatanrevisisub} where kodekeg='%s'",
					   $e_kodekeg
					   );
		$result = db_query($sql);
...
function kegiatanrevisi_edit_form_submit($form, &$form_state) {
    if($form_state['clicked_button']['#value'] == $form_state['values']['submitrek']) {
       $e_kodekeg = $form_state['values']['e_kodekeg'];
	   $form_state['redirect'] = 'apbd/kegiatanrevisi/rekening/' . $e_kodekeg ;
		//drupal_set_message('Next');
		
    } elseif ($form_state['clicked_button']['#value'] == $form_state['values']['submitprint']) {
       $e_kodekeg = $form_state['values']['e_kodekeg'];
	   $form_state['redirect'] = 'apbd/kegiatanrevisi/print/' . $e_kodekeg  ;
		//drupal_set_message('Next');
		...
		if ($e_kodekeg=='')
		{
			...
			$sql = sprintf("update {kegiatanrevisi} set adminok=1, kegiatan='%s', lokasi='%s', programsasaran='%s', programtarget='%s', masukansasaran='%s', 		
					masukantarget='%s', hasilsasaran='%s', hasiltarget='%s', keluaransasaran='%s', keluarantarget='%s', totalsebelum='%s', 
					totalsesudah='%s', plafon='%s', sumberdana1='%s', sumberdana1rp='%s', sumberdana2='%s', sumberdana2rp='%s', waktupelaksanaan='%s', 
					...
					db_escape_string($totalsesudah),db_escape_string($plafon),					  					  
		...
		if ($res) { 
			...
			if ($kodeberubah) {
				//Update anggperkeg
				$sql = sprintf("update {anggperkegrevisi} set kodekeg='%s' where kodekeg='%s'",
								db_escape_string($kodekeg),
								$e_kodekeg);		
				$res = db_query($sql);

				//Update anggperkegdetil
				$sql = sprintf("update {anggperkegdetilrevisi} set kodekeg='%s' where kodekeg='%s'",
								db_escape_string($kodekeg),
								$e_kodekeg);		
				$res = db_query($sql);

				//Update revisi
				$sql = sprintf("update {kegiatanrevisiperubahan} set kodeuk='%s', kodekeg='%s' where kodekeg='%s'",
								db_escape_string($kodeuk),db_escape_string($kodekeg),
								$e_kodekeg);		
				$res = db_query($sql);
			...
			//Update revisi
			$sql = sprintf("update {kegiatanrevisiperubahan} set kodeuk='%s' where kodekeg='%s'", db_escape_string($kodeuk), $e_kodekeg);		
			$res = db_query($sql);
