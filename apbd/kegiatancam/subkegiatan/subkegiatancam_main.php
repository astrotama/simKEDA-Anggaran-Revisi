<?php
function subkegiatancam_main($arg=NULL, $nama=NULL) {
    if ($arg)
    ...
    $sql = 'select id,kodekeg,uraian,lokasi,total,totalsebelum,totalsesudah from {kegiatankecsub}' . $where;
    $fsql = sprintf($sql, addslashes($nama));
    $limit = 13;
    //echo $fsql;
    $countsql = "select count(*) as cnt from {kegiatankecsub}" . $where;
    ...
    if ($result) {
        while ($data = db_fetch_object($result)) {
			...
			if (user_access('kegiatancam edit'))
				$editlink .= l("<img src='/files/button-edit.png' title='Edit data'>", 'apbd/kegiatancam/subkegiatan/edit/' . $data->kodekeg . "/" . $data->id, array('html'=>TRUE)) . '&nbsp;';
			if (user_access('kegiatancam penghapusan'))
                $editlink .=l("<img src='/files/button-delete.png' title='Hapus data'>", 'apbd/kegiatancam/subkegiatan/delete/' . $data->kodekeg . "/" . $data->id, array('html'=>TRUE));
            $no++;
            $rows[] = array (
	...
	//$ptitle = db_escape_string($arg) . ' - ' ;
	$pquery = sprintf("select kegiatan, concat_ws(' ', concat(p.kodeu,p.np), u.kodedinas, k.nomorkeg) as koderesmi  from {kegiatankec} k left join {unitkerja} u on ( k.kodeuk=u.kodeuk) left join {program} p on (k.kodepro = p.kodepro)  where kodekeg='%s'", db_escape_string($arg));
	$pres = db_query($pquery);	
	if ($data = db_fetch_object($pres))
		$ptitle = $data->koderesmi . ' - ' . $data->kegiatan;
	$ptitle =l($ptitle, 'apbd/kegiatancam/anggaran/' . $arg, array('html'=>true));
    $output .= theme_box($ptitle, theme_table($header, $rows));
	if (user_access('kegiatancam tambah'))
		$output .= l("<img src='/files/button-add.png' title='Tambah data baru'>", 'apbd/kegiatancam/subkegiatan/edit/' . db_escape_string($arg) , array('html'=>TRUE)) ;
//	if (user_access('kegiatancam pencarian'))		
//        $output .= l("<img src='/files/button-search.png' title='Pencarian data'>", 'apbd/kegiatancam/subkegiatan/find/' , array('html'=>TRUE)) ;
