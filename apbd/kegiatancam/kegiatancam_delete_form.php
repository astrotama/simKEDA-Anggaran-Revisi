...
function kegiatancam_delete_form() {
...
function kegiatancam_delete_form_validate($form, &$form_state) {
}
function kegiatancam_delete_form_submit($form, &$form_state) {
    if ($form_state['values']['confirm']) {
        $kodekeg = $form_state['values']['kodekeg'];
        $sql = 'DELETE FROM {kegiatankec} WHERE kodekeg=\'%s\' ';
        $res = db_query(db_rewrite_sql($sql), array($kodekeg));
        if ($res) {
            $query = sprintf("DELETE FROM {kegiatankecsub} where kodekeg='%s'", db_escape_string($kodekeg));
            db_query($query);
