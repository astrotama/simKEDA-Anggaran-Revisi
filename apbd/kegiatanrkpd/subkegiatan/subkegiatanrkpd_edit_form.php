...
function subkegiatanrkpd_edit_form(){
	drupal_add_css('files/css/kegiatancam.css');
	drupal_set_title('Sub Kegiatan RKPD');
    $form['formdata'] = array (
        '#type' => 'fieldset',
        '#title'=> 'Edit Data Sub Kegiatan RKPD',
        '#collapsible' => TRUE,
        '#collapsed' => FALSE,        
    );
	$kodekeg=arg(4);
    ...
    if (isset($id))
    {
        ...
        $sql = 'select id,kodekeg,uraian,lokasi,total,totalsebelum,totalsesudah from {kegiatanrkpdsub} where id=\'%s\'';
		...
		if (!user_access('kegiatanrkpd tambah'))
			drupal_access_denied();
		$form['formdata']['#title'] = 'Tambah Sub Kegiatan RKPD';
		$id = '';
	}
    ...
    $form['formdata']['submit'] = array (
        '#type' => 'submit',
		'#suffix' => "&nbsp;<a href='/apbd/kegiatanrkpd/subkegiatan/" . $kodekeg . "' class='btn_blue' style='color: white'>Batal</a>",
        '#value' => 'Simpan'
    );
    
    return $form;
}
function subkegiatanrkpd_edit_form_validate($form, &$form_state) {
//$id = arg(3);
//    if (!isset($id)) {
...
function subkegiatanrkpd_edit_form_submit($form, &$form_state) {
    ...
    if ($e_id=='')
    {
        $sql = 'insert into {kegiatanrkpdsub} (kodekeg,uraian,lokasi,total,totalsebelum,totalsesudah) values(\'%s\', \'%s\', \'%s\', \'%s\', \'%s\', \'%s\')';        
        $res = db_query(db_rewrite_sql($sql), array($e_kodekeg, $uraian, $lokasi, $total, $totalsebelum, $totalsesudah));
    } else {
        $sql = 'update {kegiatanrkpdsub} set uraian=\'%s\', lokasi=\'%s\', total=\'%s\', totalsebelum=\'%s\', totalsesudah=\'%s\' where id=\'%s\' ';
        $res = db_query(db_rewrite_sql($sql), array($uraian, $lokasi, $total, $totalsebelum, $totalsesudah, $e_id));
    }
	
	//UPDATE TOTAL KEGIATAN
	$sql = sprintf("select sum(total) as totalsub from {kegiatanrkpdsub} where kodekeg='%s'",
		   $e_kodekeg
		   );
	$result = db_query($sql);
	if ($data = db_fetch_object($result)) {		
		...
		$sql = sprintf("update {kegiatanrkpd} set totalpenetapan='%s',total='%s' where kodekeg='%s'",
    ...
    drupal_goto('apbd/kegiatanrkpd/subkegiatan/' . $e_kodekeg);    
}
?>