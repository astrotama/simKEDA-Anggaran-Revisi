...
function subkegiatanrkpd_delete_form() {
        ...
        if ($res) {
            ...
            if ($data) {
                ...
                return confirm_form($form,
                                    ...
                                    'apbd/kegiatanrkpd/subkegiatan/' . $data->kodekeg,
...
function subkegiatanrkpd_delete_form_validate($form, &$form_state) {
}
function subkegiatanrkpd_delete_form_submit($form, &$form_state) {
    if ($form_state['values']['confirm']) {
        $id = $form_state['values']['id'];
        $kodekeg = $form_state['values']['kodekeg'];
        
        $sql = 'DELETE FROM {kegiatanrkpdsub} WHERE id=\'%s\' ';
        $res = db_query(db_rewrite_sql($sql), array($id));
        if ($res) {
            drupal_set_message('Penghapusan berhasil dilakukan');
            drupal_goto('apbd/kegiatanrkpd/subkegiatan/' . $kodekeg);
        }
