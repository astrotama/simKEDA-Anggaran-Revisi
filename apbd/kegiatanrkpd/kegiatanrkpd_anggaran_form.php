<?php
function kegiatanrkpd_anggaran_form(){

    $kodekeg = arg(3);
	if (!isset($kodekeg))
		drupal_access_denied();

	if (!user_access('kegiatanrkpd anggaran'))
		drupal_access_denied();

	
    $form['formdata'] = array (
        '#type' => 'fieldset',
        '#title'=> 'Data Kegiatan RKPD - [' . $kodekeg . ']',
        '#collapsible' => TRUE,
        '#collapsed' => FALSE,        
    );
	...
	drupal_set_title('Kegiatan RKPD - Isian Anggaran Kegiatan');
	...
	$sql = 'select k.kodekeg, k.tahun, k.kodepro, k.kodeuk, k.kodeuktujuan, k.sifat, k.kegiatan, k.lokasi, k.sasaran, k.target, k.totalsebelum, k.total, k.targetsesudah, k.nilai, k.lolos, k.asal, k.kodekec, k.apbdkab, k.apbdprov, k.apbdnas, k.kodebid, k.dekon, k.apbp, k.apbn, k.kodesuk, k.totalsebelum2, k.totalsebelum3, k.totalpenetapan, k.sumberdana, k.pnpm, p.program, concat_ws(\' \', concat(p.kodeu,p.np), u.kodedinas, k.nomorkeg) as koderesmi from {kegiatanrkpd} k left join {program} p on (k.kodepro = p.kodepro) left join {unitkerja} u on (k.kodeuk = u.kodeuk) where k.kodekeg=\'%s\'' . $customwhere;
	...
	if ($res) {
		...
		if ($data) {
			...
			$sifat = $data->sifat;
			$kegiatan = $data->kegiatan . '&nbsp;&nbsp;' . l('Sub Kegiatan', 'apbd/kegiatanrkpd/subkegiatan/' . $kodekeg, array('html'=>TRUE, 'attributes'=>array('class'=>'btn_blue', 'style'=>'color:#ffffff;')));
			$lokasi = $data->lokasi;
			...
			$form['formdata']['#title'] = 'Data Kegiatan RKPD - [' . $data->koderesmi . ']';
...
function kegiatanrkpd_anggaran_form_validate($form, &$form_state) {
...
function kegiatanrkpd_anggaran_form_submit($form, &$form_state) {
	...
	$sql = sprintf("update {kegiatanrkpd} set total='%s', totalsebelum ='%s', totalsebelum2='%s', totalpenetapan='%s', lolos='%s', apbdkab='%s', apbdprov='%s', apbdnas='%s', apbp='%s', apbn='%s', dekon='%s', sumberdana='%s'  where kodekeg='%s'",
