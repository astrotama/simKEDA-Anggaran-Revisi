<?php
function renjaskpdnonapbd_main() {
...
drupal_add_css('files/css/kegiatancam.css');
	...
	if (!isset($tahun)) 
		return drupal_get_form('renjaskpdnonapbd_form');
	...
	if (isset($exportpdf) && ($exportpdf=='pdf'))  {
		...
		renjaskpdnonapbd_exportexcel($kodeuk, $tahun);
		
	} else {
		//PDF
		$urlpdf = 'apbd/laporan/renjaskpdnonapbd/' . $kodeuk . '/' . $tahun . '/pdf';
		$urlxls = 'apbd/laporan/renjaskpdnonapbd/' . $kodeuk . '/' . $tahun . '/xls';
		$output .= drupal_get_form('renjaskpdnonapbd_form');
...
function GenReportForm($print=0) {
	...
	$rows= array();
	$headers1[] = array (array ('data'=>'PENJABARAN BELANJA PPAS PER URUSAN - SKPD', 'width'=>'870px', 'colspan'=>'6', 'style' =>'border: 0px solid white;font-weight:900;font-size:1.3em;text-align:center;'));
	...
	if ($result) {
		...
		while ($data = db_fetch_object($result)) {                
			...
			$rows[] = array (
				...
				array('data' => apbd_fn($data->jpegawai) , 'width' => $colrekening, 'align' => 'right', 'valign'=>'top', 'style' => 'border-bottom: 1px solid black; border-right: 1px solid black; font-weight:700;font-size: 1em;'),
				array('data' => apbd_fn($data->jbarangjasa), 'width' => $colrekening, 'align' => 'right', 'valign'=>'top', 'style' => 'border-bottom: 1px solid black; border-right: 1px solid black; font-weight:700;font-size: 1em;'),
				array('data' => apbd_fn($data->jmodal), 'width' => $colrekening, 'align' => 'right', 'valign'=>'top', 'style' => 'border-bottom: 1px solid black; border-right: 1px solid black; font-weight:700;font-size: 1em;'),
			...
			while ($data_1 = db_fetch_object($result_1)) {                
				...
				$rows[] = array (
					...
					array('data' => apbd_fn($data_1->jpegawai) , 'width' => $colrekening, 'align' => 'right', 'valign'=>'top', 'style' => 'border-bottom: 1px solid black; border-right: 1px solid black; font-size: 1em;'),
					array('data' => apbd_fn($data_1->jbarangjasa), 'width' => $colrekening, 'align' => 'right', 'valign'=>'top', 'style' => 'border-bottom: 1px solid black; border-right: 1px solid black; font-size: 1em;'),
					array('data' => apbd_fn($data_1->jmodal), 'width' => $colrekening, 'align' => 'right', 'valign'=>'top', 'style' => 'border-bottom: 1px solid black; border-right: 1px solid black; font-size: 1em;'),
...
function renjaskpdnonapbd_form () {
	$form['formdata'] = array (
		'#type' => 'fieldset',
...
function renjaskpdnonapbd_form_submit($form, &$form_state) {
	//$kodeuk = $form_state['values']['kodeuk'];
	$tahun = $form_state['values']['tahun'];
	$kodeuk = $form_state['values']['kodeuk'];
	$uri = 'apbd/laporan/renjaskpdnonapbd/' . $kodeuk . '/' . $tahun;
	drupal_goto($uri);
	

}

function renjaskpdnonapbd_exportexcel($kodeuk, $tahun) {
