...
drupal_add_css('files/css/kegiatancam.css');
	...
	if (isset($exportpdf) && ($exportpdf=='pdf'))  {
		...
		$url = 'apbd/laporan/rka/rekapaggblprogram/'.$revisi.'/'. $kodeuk . '/' . $topmargin . "/pdf";
...
function rekapaggblprogram_form_submit($form, &$form_state) {
	...
	if($form_state['clicked_button']['#value'] == $form_state['values']['tampilkan']) 
		$uri = 'apbd/laporan/rka/rekapaggblprogram/'.$revisi.'/' . $kodeuk . '/'. $topmargin . '/' ;
	else
		$uri = 'apbd/laporan/rka/rekapaggblprogram/'.$revisi.'/' . $kodeuk . '/'. $topmargin . '/pdf' ;
	drupal_goto($uri);
