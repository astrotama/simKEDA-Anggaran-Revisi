...
function GenReportFormHeader($kodeuk, $tipedok) {
	...
	if ($tipedok=='dpa') {
		$rowskegiatan[]= array ( 
							 array('data' => 'DOKUMEN PELAKSANAAN ANGGARAN',  'width'=> '350px', 'colspan'=>'4', 'style' => 'border-left: 1px solid black; border-right: 1px solid black; border-top: 2px solid black; ; text-align:center;'),
...
function rekapaggblprogramtw_form_submit($form, &$form_state) {
	...
	if($form_state['clicked_button']['#value'] == $form_state['values']['tampilkan'])
		$uri = 'apbd/laporan/rka/rekapaggblprogramtw/' . $kodeuk . '/'. $topmargin . '/' . $tipedok ;
	else
		$uri = 'apbd/laporan/rka/rekapaggblprogramtw/' . $kodeuk . '/'. $topmargin .  '/' . $tipedok . '/pdf' ;
	drupal_goto($uri);
