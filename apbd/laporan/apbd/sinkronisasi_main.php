...
function GenReportFormHeader($prov) {
	...
	$rowsjudul[] = array (array ('data'=>'Sinkronisasi Kebijakan Pemerintah Kabupaten Jepara dalam', 'width'=>'875px', 'colspan'=>'7', 'style' =>'border:none; font-weight:900; font-size:1.3em; text-align:center;'));
	$rowsjudul[] = array (array ('data'=>'Rancangan Peraturan Daerah tentang ' . $lbl_perubahan . 'APBD Tahun Anggaran 2016 dan', 'width'=>'875px', 'colspan'=>'7', 'style' =>'border:none; font-weight:900; font-size:1.3em; text-align:center;'));
...
function GenReportFormContent($prov) {
	...
	$headersrek[] = array (
						 ...
						 array('data' => 'Alokasi Anggaran Belanja dalam Rancangan ' . $lbl_perubahan . 'APBD',  'width' => '270px', 'colspan'=>'3', 'style' => 'border-bottom: 1px solid black; border-right: 1px solid black; border-top: 2px solid black; text-align:center;'),
	...
	if ($resultbid) 	{
		while ($databid = db_fetch_object($resultbid)) {
			...
			$where = sprintf(' where k.inaktif=0 and p.kodebid=\'%s\'', db_escape_string($databid->kodebid));
			$sql = 'select k.jenis,sum(k.total' . $kolom_anggaran . ') totalbid from {prioritas' . $prov . '} p inner join {prioritasprogram' . $prov . '} pp on p.prioritasno= pp.prioritasno inner join ' . $tabel_kegiatan . ' k on pp.kodepro=k.kodepro ' . $where . ' group by k.jenis';
			//drupal_set_message($sql);
			$resultbidnum = db_query($sql);	
			if ($resultbid) 	{
				while ($databidnum = db_fetch_object($resultbidnum)) {
...
function sinkronisasi_form_submit($form, &$form_state) {
	...
	if($form_state['clicked_button']['#value'] == $form_state['values']['tampilkan']) 
		$uri = 'apbd/laporan/apbd/sinkronisasi/' . $prov . '/' . $topmargin . '/' . $hal1 ;
	else
		$uri = 'apbd/laporan/apbd/sinkronisasi/' . $prov . '/' . $topmargin . '/' . $hal1 . '/pdf' ;
	drupal_goto($uri);
