...
function GenReportFormHeader($kodeuk) {
	...
	$rowsjudul[] = array (array ('data'=> 'DAFTAR KEGIATAN APBD PERUBAHAN TAHUN 2016', 'width'=>'535px', 'colspan'=>'3', 'style' =>'border:none; font-weight:900; font-size:1.3em; text-align:center;'));
...
function GenReportFormContent($kodeuk,$revisi) {
	...
	$where = " and k.kodepro>'010' and k.kodekeg in (select kodekeg from {anggperkegperubahan" . $str_table . "} where mid(kodero,1,2)='52' or mid(kodero,1,2)='53') ";
						 
	$sql = 'select k.kodekeg,k.kegiatan,k.lokasi,uk.namasingkat,k.totalp total from {kegiatanperubahan' . $str_table . '} k inner join {unitkerja} uk on k.kodeuk=uk.kodeuk where uk.kelompok<=1 and k.jenis=2 and k.totalp>0 and k.inaktif=0 ' . $whereuk . $where . ' order by k.total';
	...
	if ($result) {
		...
		$rowsrek[] = array (
							 ...
							 array('data' => '<strong>TOTAL</strong>',  'width' => '170px', 'style' => 'border-top: 1px solid black; border-bottom: 1px solid black; text-align:left;'),
							 array('data' => '',  'width' => '100px', 'style' => ' border-top: 1px solid black; border-bottom: 1px solid black; text-align:left;'),
							 array('data' => '',  'width' => '150px', 'style' => ' border-right: 1px solid black;border-top: 1px solid black; border-bottom: 1px solid black; text-align:left;'),
							 array('data' => '<strong>' . apbd_fn($total) . '</strong>',  'width' => '90px', 'style' => ' border-right: 1px solid black; border-top: 1px solid black; border-bottom: 1px solid black; text-align:right;'),
							 );
