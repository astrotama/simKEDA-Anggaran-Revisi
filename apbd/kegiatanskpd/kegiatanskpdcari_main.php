...
function GenDataView($kodeuk , $sumberdana , $statusisi , $kodesuk , $statustw , 
	...
	if (($rekening == '') and ($rincian =='')) {
		...
		$sql = "select distinct k.kodekeg,k.nomorkeg,k.tahun,k.kodepro,k.kodeuk, k.kegiatan,k.lokasi, 				
				k.programtarget,k.total,k.plafon,u.namasingkat, k.adminok, k.sumberdana1 sumberdana, k.inaktif from {kegiatanskpd} k inner join {unitkerja} u on (k.kodeuk=u.kodeuk) inner join {anggperkeg} a on k.kodekeg=a.kodekeg " . $where;
		$fsql = $sql;
		
		//echo $fsql;
		
		$countsql = "select count(*) as cnt from {kegiatanskpd} k inner join {anggperkeg} a on k.kodekeg=a.kodekeg " . $where;
		$result = pager_query($fsql . $tablesort, $limit, 0, $countsql);
		...
		$where = ' where ' . sprintf(' k.tahun=%s ', $tahun) . $qlike ;
		$sql = "select distinct  k.kodekeg,k.nomorkeg,k.tahun,k.kodepro,k.kodeuk,k.kegiatan,k.lokasi, 				
				k.programtarget,k.total,k.plafon,u.namasingkat, k.adminok, k.sumberdana1 sumberdana, k.inaktif from {kegiatanskpd} k inner join {unitkerja} u on (k.kodeuk=u.kodeuk) inner join {anggperkeg} a on k.kodekeg=a.kodekeg inner join {anggperkegdetil} d on a.kodekeg=d.kodekeg and a.kodero=d.kodero " . $where;
		$fsql = $sql;
		
		$countsql = "select count(*) as cnt from {kegiatanskpd} k inner join {anggperkeg} a on k.kodekeg=a.kodekeg inner join {anggperkegdetil} d on a.kodekeg=d.kodekeg and a.kodero=d.kodero " . $where;
		$result = pager_query($fsql . $tablesort, $limit, 0, $countsql);
    ...
    $btn .= "&nbsp;" . l("Cetak", 'apbd/kegiatanskpdcari/filter/' . $kodeuk . '/' . $sumberdana . '/' . $statusisi . '/' . $kodesuk . '/'. $statustw . '/' . $statusinaktif . '/' . $jenis . '/' . $kegiatan . '/' . $rekening . '/' . $rincian . '/pdf' , array ('html' => true, 'attributes'=> array ('class'=>'btn_blue', 'style'=>'color:white;'))) ;
