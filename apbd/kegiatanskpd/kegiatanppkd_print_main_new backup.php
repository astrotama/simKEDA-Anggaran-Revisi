...
function GenReportFormContent() {
	...
	$sql = 'select l.kodek,l.uraian,sum(k.jumlah) jumlahx, sum(k.jumlahp) jumlahxp from {anggperkegperubahan} k  inner join {kelompok} l on mid(k.kodero,1,2)=l.kodek ' . $where;
	...
	if ($resultkel) {
		while ($datakel = db_fetch_object($resultkel)) {
			...
			$sql = 'select mid(k.kodero,1,3) kodej,j.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp from {anggperkegperubahan} k  left join {jenis} j on mid(k.kodero,1,3)=j.kodej ' . $where;
			...
			if ($resultjenis) {
				while ($datajenis = db_fetch_object($resultjenis)) {
					...
					//OBYEK
					$sql = 'select mid(k.kodero,1,5) kodeo,o.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp from {anggperkegperubahan} k  left join {obyek} o on mid(k.kodero,1,5)=o.kodeo where mid(k.kodero,1,3)=\'%s\'';
					...
					if ($resultobyek) {
						while ($dataobyek = db_fetch_object($resultobyek)) {
							...
							//REKENING
							$sql = 'select kodero,uraian,sum(jumlah) jumlahx ,sum(jumlahp) jumlahxp from {anggperkegperubahan} k where mid(k.kodero,1,5)=\'%s\'';
							...
							if ($result) {
								...
								$rowsrek[] = array (
									...
									if ($datajenis->kodej<'516') {
										...
										//DETIL
										$sql = 'select iddetil,uraian,unitjumlah,unitsatuan,volumjumlah, volumsatuan,harga,total from {anggperkegdetilperubahan} where kodero=\'%s\' order by kodekeg,iddetil';
										...
										//KEGIATAN
										$sql = 'select k.kodekeg,k.kegiatan,a.jumlah,a.jumlahp from {kegiatanperubahan} k inner join {anggperkegperubahan} a on k.kodekeg=a.kodekeg where a.kodero=\'%s\' order by kegiatan';
										$fsql = sprintf($sql, db_escape_string($data->kodero));
...
function GenReportFormFooter() {
	...
	if ($tipedok='dpa') {
		...
		$pquery = "select sum(tw1p) tw1t,sum(tw2p) tw2t,sum(tw3p) tw3t,sum(tw4p) tw4t from kegiatanperubahan
where kodekeg in (select kodekeg from anggperkegperubahan where left(kodero,3) between '512' and '518')";
