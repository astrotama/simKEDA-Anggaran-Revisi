<?php
function kegiatanskpd_printkas_main() {
...
drupal_add_css('files/css/kegiatancam.css');
	...
	if (isset($exportpdf) && ($exportpdf=='pdf'))  {
		...
		$pdfFile = 'anggaran-kas-' . $kodekeg . '.pdf';
		...
		$url = 'apbd/kegiatanskpd/print/'. $kodekeg . '/' . $topmargin . "/pdf";
		$output = drupal_get_form('kegiatanskpd_printkas_form');
...
function GenReportForm($print=0) {
	...
	*/
	...
	$pimpinannip='';
	$pimpinanjabatan='';
	$pquery = sprintf("select namauk from {unitkerja} u left join {kegiatanskpd} k 
	...
	$pquery = sprintf('select kegiatan, total, tw1, tw2, tw3, tw4 from {kegiatanskpd} ' . $where, db_escape_string($kodekeg));
	//drupal_set_message($pquery);
	$pres = db_query($pquery);
	if ($data = db_fetch_object($pres)) {
	...
	$rowsrek[] = array (
						 array('data' => 'JUMLAH BELANJA LANGSUNG',  'width'=> '775px',  'colspan'=>'5',  'style' => 'border-left: 1px solid black; border-bottom: 1px solid black; border-right: 1px solid black; border-top: 1px solid black; text-align:right; font-weight:bold;'),
...
function GenReportFormContent($kodekeg) {
	...
	*/
	...
	$rowsrek[] = array (
						 array('data' => 'JUMLAH BELANJA LANGSUNG',  'width'=> '775px',  'colspan'=>'6',  'style' => 'border-left: 1px solid black; border-bottom: 1px solid black; border-right: 1px solid black; border-top: 1px solid black; text-align:right; font-weight:bold;'),
						 array('data' => apbd_fn($total),  'width' => '100px', 'style' => 'border-bottom: 1px solid black; border-right: 1px solid black; border-top: 1px solid black; text-align:right; font-weight:bold;'),
						 );
	$opttbl = array('cellspacing'=>'1', 'cellpadding'=>'1', 'border' => '0');
...
function kegiatanskpd_printkas_form () {
	$form['formdata'] = array (
		'#type' => 'fieldset',
...
function kegiatanskpd_printkas_form_submit($form, &$form_state) {
	//$kodeuk = $form_state['values']['kodeuk'];
	$kodekeg = $form_state['values']['kodekeg'];
	$topmargin = $form_state['values']['topmargin'];
	$uri = 'apbd/kegiatanskpd/print/' . $kodekeg . '/'. $topmargin . '/pdf' ;
	drupal_goto($uri);
