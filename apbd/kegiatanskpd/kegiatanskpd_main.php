...
function kegiatanskpd_main($arg=NULL, $nama=NULL) {
	...
	$sql = "select k.kodekeg,k.nomorkeg,k.tahun,k.kodepro,k.kodeuk,k.kegiatan,k.jenis, k.lokasi,k.programtarget,k.total,
			k.plafon,u.namasingkat, k.isppkd,  k.adminok, k.sumberdana1 sumberdana, k.inaktif,k.dispensasi, concat_ws(' ', concat(p.kodeu,p.np), u.kodedinas, k.nomorkeg) as koderesmi from {kegiatanskpd} k left join {unitkerja} u on ( k.kodeuk=u.kodeuk) left join {program} p on (k.kodepro = p.kodepro) " . $where;
        ...
        while ($data = db_fetch_object($result)) {
			...
			if ($allowedit) 
				...
				//if ($adminok or $data->adminok) {
					...
					//if ($data->jenis==2)
					//	$editlink .= "&nbsp;" .  l('Kas', 'kasbl/' . $data->kodekeg, array('html'=>TRUE));
					//else
					//	$editlink .= "&nbsp;" .  l('Kas', 'kasgaji/' . $data->kodekeg, array('html'=>TRUE));
			...
			if ($data->jenis==2)
				...
				if ($data->isppkd)
					$editlink .= "&nbsp;" .  l('Kas', 'kasbppkd/' . $data->kodekeg, array('html'=>TRUE));
				else
					$editlink .= "&nbsp;" .  l('Kas', 'kasgaji/' . $data->kodekeg, array('html'=>TRUE));
			}
			*/
...
function kegiatanskpd_main_form() {
	...
	if (isSuperuser()) {
		...
		//if (!isVerifikator()) $dinas['00'] ='00000 - SEMUA SKPD';
...
function kegiatanskpd_main_form_submit($form, &$form_state) {
	...
	$uri = 'apbd/kegiatanskpd/filter/' . $kodeuk . '/' . $sumberdana . '/' . $statusisi . '/' . $kodesuk . '/'. $statustw . '/' . $statusinaktif . '/' . $jenis;
	drupal_goto($uri);
