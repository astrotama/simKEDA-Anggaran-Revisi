<?php
function kegiatanskpd_printdpa_main() {
...
drupal_add_css('files/css/kegiatancam.css');
	...
	if (isset($exportpdf) && ($exportpdf=='pdf'))  {
		...
		$pdfFile = 'rka-skpd-belanja-' . $kodekeg . '.pdf';
		...
		$output = drupal_get_form('kegiatanskpd_printdpa_form');
...
function GenReportForm($kodekeg) {
	...
	*/
	...
	$pquery = sprintf('select k.kodekeg, k.nomorkeg, k.tahun, k.kodepro, k.kodeuk, k.kodesuk, 
				...
				p.kodepro, p.kodeu, u.urusan, u.kodef, u.fungsi from {kegiatanskpd} k left join {program} p 
...
function kegiatanskpd_printdpa_form () {
	$form['formdata'] = array (
		'#type' => 'fieldset',
...
function kegiatanskpd_printdpa_form_submit($form, &$form_state) {
