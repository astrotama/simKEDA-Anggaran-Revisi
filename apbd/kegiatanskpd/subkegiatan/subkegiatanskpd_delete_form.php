...
function subkegiatanskpd_delete_form() {
    drupal_add_css('files/css/kegiatancam.css');	
    drupal_add_js('files/js/common.js');
    drupal_add_js("$(document).ready(function(){ updateAnchorClass('.container-inline')});", 'inline');
    $id = arg(5);
    if (isset($id)) {
        $sql = 'select id,kodekeg,uraian,lokasi,total,totalsebelum,totalsesudah from {kegiatanskpdsub} where id=\'%s\'';
        ...
        if ($res) {
            ...
            if ($data) {
                ...
                $form['formdata'] = array (
                    '#type' => 'fieldset',
                    '#title'=> 'Hapus Sub Kegiatan Renja SKPD',
                    '#collapsible' => TRUE,
                    '#collapsed' => FALSE,        
                );
                ...
                return confirm_form($form,
                                    ...
                                    'apbd/kegiatanskpd/subkegiatan/' . $data->kodekeg,
...
function subkegiatanskpd_delete_form_validate($form, &$form_state) {
}
function subkegiatanskpd_delete_form_submit($form, &$form_state) {
    if ($form_state['values']['confirm']) {
        $id = $form_state['values']['id'];
        $kodekeg = $form_state['values']['kodekeg'];
        
        $sql = 'DELETE FROM {kegiatanskpdsub} WHERE id=\'%s\' ';
        $res = db_query(db_rewrite_sql($sql), array($id));
        if ($res) {
            drupal_set_message('Penghapusan berhasil dilakukan');
            drupal_goto('apbd/kegiatanskpd/subkegiatan/' . $kodekeg);
        }
