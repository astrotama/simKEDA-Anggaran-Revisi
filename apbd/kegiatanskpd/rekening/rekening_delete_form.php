...
function rekening_delete_form() {
    ...
    if (isset($kodero)) {
        ...
        $sql = 'select kodekeg,kegiatan from {kegiatanskpd} where kodekeg=\'%s\'';
    ...
    if ($form_state['values']['confirm']) {
		...
		if ($kodero=='xx') {
			...
			if ($res) {
				...
				//nolkan anggarannya;
				$sql = 'UPDATE {kegiatanskpd} SET total=0 WHERE kodekeg=\'%s\'';
				$res = db_query(db_rewrite_sql($sql), array($kodekeg));
			...
			if ($res) {
				...
				$sql = 'UPDATE {kegiatanskpd} SET total=\'%s\' WHERE kodekeg=\'%s\'';
