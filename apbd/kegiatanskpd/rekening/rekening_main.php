...
function rekening_main($arg=NULL, $nama=NULL) {
   ...
   if ($arg) {
    ...
    $sql = 'select kodekeg,kodero,uraian,jumlahsebelum,jumlah,jumlahsesudah from {anggperkeg}' . $where . $tablesort;
	...
	$pquery = sprintf("select kegiatan, total, plafon, concat_ws(' ', concat(p.kodeu,p.np), u.kodedinas, k.nomorkeg) as koderesmi  from {kegiatanskpd} k left join {unitkerja} u on ( k.kodeuk=u.kodeuk) left join {program} p on (k.kodepro = p.kodepro)  where kodekeg='%s'", db_escape_string($arg));
