...
function subkegiatanppa_edit_form(){
    $form['formdata'] = array (
        '#type' => 'fieldset',
        '#title'=> 'Edit Data Sub Kegiatan PPA',
        '#collapsible' => TRUE,
        '#collapsed' => FALSE,        
    );
	$kodekeg=arg(4);
	...
	drupal_set_title('Sub Kegiatan PPA');
    ...
    if (isset($id))
    {
		...
		//	drupal_access_denied();
		$form['formdata']['#title'] = 'Tambah Sub Kegiatan PPA';
		$id = '';
	}
    ...
    $form['formdata']['submit'] = array (
        '#type' => 'submit',
		'#suffix' => "&nbsp;<a href='/apbd/kegiatanppa/subkegiatan/" . $kodekeg . "' class='btn_blue' style='color: white'>Batal</a>",
        '#value' => 'Simpan'
    );
    
    return $form;
}
function subkegiatanppa_edit_form_validate($form, &$form_state) {
//$id = arg(3);
//    if (!isset($id)) {
...
function subkegiatanppa_edit_form_submit($form, &$form_state) {
    ...
    if ($e_id=='')
    {
        $sql = 'insert into {kegiatanppasub} (kodekeg,uraian,lokasi,total,totalsebelum,totalsesudah) values(\'%s\', \'%s\', \'%s\', \'%s\', \'%s\', \'%s\')';        
        $res = db_query(db_rewrite_sql($sql), array($e_kodekeg, $uraian, $lokasi, $total, $totalsebelum, $totalsesudah));
    } else {
        $sql = 'update {kegiatanppasub} set uraian=\'%s\', lokasi=\'%s\', total=\'%s\', totalsebelum=\'%s\', totalsesudah=\'%s\' where id=\'%s\' ';
	...
	//Disable dulu 
	/*
	$sql = sprintf("select sum(total) as totalsub from {kegiatanppasub} where kodekeg='%s'",
		   $e_kodekeg
		   );
	$result = db_query($sql);
	if ($data = db_fetch_object($result)) {		
		...
		$sql = sprintf("update {kegiatanppa} set totalpenetapan='%s',total='%s' where kodekeg='%s'",
	...
	*/
    ...
    drupal_goto('apbd/kegiatanppa/subkegiatan/' . $e_kodekeg);    
}
?>