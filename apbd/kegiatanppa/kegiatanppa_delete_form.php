...
function kegiatanppa_delete_form() {
...
function kegiatanppa_delete_form_validate($form, &$form_state) {
}
function kegiatanppa_delete_form_submit($form, &$form_state) {
    if ($form_state['values']['confirm']) {
        $kodekeg = $form_state['values']['kodekeg'];
        $sql = 'DELETE FROM {kegiatanppa} WHERE kodekeg=\'%s\' ';
        $res = db_query(db_rewrite_sql($sql), array($kodekeg));
        if ($res) {
