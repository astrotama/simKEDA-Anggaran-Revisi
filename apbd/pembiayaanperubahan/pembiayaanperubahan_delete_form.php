...
function pembiayaanperubahan_delete_form() {
    ...
    if (isset($kodero)) {
        $sql = 'select tahun,kodero,uraian,jumlah from {anggperdaperubahan} where tahun=\'%s\' and kodero=\'%s\'';
...
function pembiayaanperubahan_delete_form_validate($form, &$form_state) {
}

function pembiayaanperubahan_delete_form_submit($form, &$form_state) {
    if ($form_state['values']['confirm']) {
        $kodero = $form_state['values']['kodero'];
		$tahun = $form_state['values']['tahun'];
		$jumlah = $form_state['values']['jumlah'];
        	
        $sql = 'DELETE FROM {anggperdaperubahan} where tahun=\'%s\' and kodero=\'%s\'';
        $res = db_query(db_rewrite_sql($sql), array($tahun, $kodero));
        if ($res) {
			
			//Delete detilnya
			$sql = 'DELETE FROM {anggperdadetilperubahan} WHERE tahun=\'%s\' and kodero=\'%s\'';
			$res = db_query(db_rewrite_sql($sql), array($tahun, $kodero));
