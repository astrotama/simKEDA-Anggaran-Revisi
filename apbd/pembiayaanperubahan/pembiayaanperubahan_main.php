<?php
function pembiayaanperubahan_main($arg=NULL, $nama=NULL) {
	...
	//$output .= drupal_get_form('pembiayaanperubahan_transfer_form');
	$output .= drupal_get_form('pembiayaanperubahan_main_form');
	$header = array (
		...
		array('data' => 'Dasar Hukum','width' => '500px', 'field'=> 'sasaran', 'valign'=>'top'),
	...
	$sql = "select tahun,kodero,uraian,jumlah,jumlahsebelum,jumlahp,periode,ketrekening from {anggperdaperubahan} " . $where;
			...
			if ($allowedit) {
				if (user_access('kegiatanrkpd penghapusan')) {
					if ($data->periode==$periodeaktif)
						$editlink .=l('Hapus', 'apbd/pembiayaanperubahan/delete/' . $data->kodero, array('html'=>TRUE));
					else
						$editlink = 'Hapus';
				}
			}
	...
	$status = 0;
	$record = 0;
	$btn .= l('Cetak', 'apbd/pembiayaan/print/9/' . $kodek, array ('html' => true, 'attributes'=> array ('class'=>'btn_blue', 'style'=>'color:white;')));
...
function pembiayaanperubahan_main_form() {
	$form['formdata'] = array (
		'#type' => 'fieldset',
		'#title'=> 'Pilihan Data',
		'#collapsible' => TRUE,
...
function pembiayaanperubahan_main_form_submit($form, &$form_state) {
	$kodek= $form_state['values']['kodek'];
	$_SESSION['kodekpembiayaan'] = $kodek;

	$uri = 'apbd/pembiayaanperubahan/filter/' . $kodek ;
	drupal_goto($uri);
	
}


function pembiayaanperubahan_exportexcel($tahun) {
